# Package hann_window_4096 as a reusable Vivado custom IP.
#
# Run from the repository root with:
#   vivado -mode batch -source tools/package_hann_window_ip.tcl
#
# This script packages only the synthesizable Hann window RTL and its generated
# coefficient file. It does not connect the IP to the current block design.
...
set ip_root_dir        [file normalize "$ip_repo_dir/hann_window_4096_1_0"]
set packager_dir       [file normalize "$main_project_root/.hann_ip_packager_tmp"]
...
# Use a small temporary project so the packaged IP does not accidentally include
# unrelated block design files, previous simulation files, or other project RTL.
create_project hann_window_ip_packager $packager_dir -part $part_name -force
set_property target_language VHDL [current_project]

add_files -norecurse [list \
    "$source_dir/hann_window_4096.vhd" \
    "$source_dir/hann_window.mem" \
]

set_property file_type VHDL [get_files "$source_dir/hann_window_4096.vhd"]
...
# Infer clock/reset interfaces for cleaner block-design integration later. The
# streaming sample ports remain ordinary scalar/vector ports for now.
...
# Register the shared local IP repository with the main project. The block
# design is intentionally left untouched.
open_project $main_project_path
set_property ip_repo_paths $ip_repo_dir [current_project]
update_ip_catalog
close_project

puts "Packaged hann_window_4096 IP at: $ip_root_dir"
